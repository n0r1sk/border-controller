...
func updatepdns(config T) {
	var rrset Recordsets
	var rs Recordset
	var r Record

	r.Content = config.Pdns.Ip_address
	r.Disabled = false

	rs.Name = config.Pdns.Domain_prefix + "." + config.Pdns.Domain_zone + "."
	...
	recordsetsreplace(config.Pdns.Api_url, config.Pdns.Api_key, config.Pdns.Domain_zone, rrset)
}
